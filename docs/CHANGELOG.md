...
Two additions: subscription logins multiply via a provider's optional `authHome` (the dir its codex/claude login lives in — several accounts of one provider ride side by side), and ODCP context pruning joins RTK in the token-saver layer (where RTK shrinks each tool output's size, ODCP drops whole superseded content — repeated identical calls and stale errored ones).
...
- `awerouter config login claude <dir>` / `config logout claude <dir>` (and codex's CODEX_HOME-aware hint): manage one account's login in its authHome; each claude store refreshes independently.
