...
**L4 问：刚发生的事情，其后果值得强模型处理吗？** 结构看不到*决定*编辑的那一轮——等到有工具调用可读时，决策已经做完了。结构能做的是反应。代码刚被改写的下一轮是审查轮：核对 diff、决定下一个文件、向用户交代。那一轮走 pro；flash 起草，pro 审查。它是后果检查点，不是难度猜测，也是四层里最弱的信号——这正是它排在最后的原因。
